        ...
        >
          ...
          <el-table-column label="操作" width="120">
            <template slot-scope="scope">
              <el-button plain type="danger" size="mini" @click="handleEdit(scope.row.JobID)">删除岗位</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div class="entry" v-show="step === 1">
        ...
        >
          ...
          <el-form-item label="企业名称">
            <el-input style="width: 150px" disabled :placeholder="holder"></el-input>
          </el-form-item>
          <el-form-item label="职位类别">
            <el-radio-group v-model="form.type">
              <el-radio :label="item.name" name="type" v-for="item in typeOp" v-bind:key="item.id"></el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="岗位名称">
...
export default {
  data() {
    return {
      step: 0,
      holder: JSON.parse(localStorage.getItem("jw_ent_file")).CompanyCode,
      loading: false,
      form: {
        name: "",
        type: "",
    ...
    handleEdit(row) {
      this.$confirm("删除该招聘信息后,求职者将无法再看到,是否继续?", "提示", {
        ...
        this.axios({
          ...
          headers: {
            "Authorization": JSON.parse(localStorage.getItem("jw_ent_file")).authorization
          },
          data: {
            "JobID": row
          }
