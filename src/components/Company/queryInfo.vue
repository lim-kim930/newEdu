...
export default {
  ...
  methods: {
    saveImg(val) {
      ...
      // 先获取你要转换为img的dom节点
      var node = document.querySelector(val);//传入的id名称
      ...
      html2canvas(node, {
        ...
        this.url = canvas.toDataURL();//这里上面不设值cors会报错
    ...
    query() {
      this.btnShow = false;
      this.table_height = "";
      if (this.input.indexOf("https://api.hduhelp.com/gormja_wrapper/share/verify?fileID=") === -1) {
        this.$message.error("请输入正确的校验链接");
        return;
      }
      this.loading = true;
      this.axios({
        ...
        .then((response) => {
          ...
          this.scoreDataPart3 = [];
          this.scoreDataPart4 = [];
          var date = new Date(response.data.data.ShareFile.expired_at).toJSON();
