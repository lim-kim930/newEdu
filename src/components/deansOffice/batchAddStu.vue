  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-table-column prop="staff_id" label="学号" width="250">
        </el-table-column>
        <el-table-column prop="staff_name" label="姓名" width="250">
        </el-table-column>
        <el-table-column prop="enrollmentYear" label="入学年份" width="150" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="schoolYear" label="学制" width="150" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="learnType" label="学习类型" width="150" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="educationalLevel" label="培养层次" show-overflow-tooltip>
        </el-table-column>
      </el-table>
      <div style="margin-top: 20px">
        <el-button @click="toggleSelection()">取消选择</el-button>
        <el-button type="primary" @click="submitForm()" :disabled="isDisabled" style="width: 150px;"><i class="el-icon-circle-plus el-icon--left"></i>批量导入</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      for(var i = 0;i<this.multipleSelection.length;i++){
        this.params.push(this.multipleSelection[i].staff_id)
      }
      console.log(this.params);
      this.axios({
        method: "post",
        url: "https://api-vnt.fdkevin.xyz/addStudentFromDB",
        headers: { Authorization: keystore, code: code },
        data: {addStudentInfo: this.params},
      })
      // .then(function () {
      //   that.isDisabled = false;
      //   that.loading = false;
      //   that.$alert("批量添加新学生成功!", "提示", {
      ...
      .then(function (response) {
          ...
          that.$confirm("此次交易号: " + response.data.txHash + "</a>","批量添加新学生成功!",
        {
          confirmButtonText: "确定",
  ...
  mounted() {
    ...
    this.axios
      .get("https://api-vnt.fdkevin.xyz/flushDB", {
        // headers: { Authorization: keystore, code: code },
      })
      .then(
        function (response) {
